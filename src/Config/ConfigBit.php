...
class ConfigBit implements EventSubscriberInterface, ContainerInjectionInterface {
  ...
  protected function processConfigBits(array $supportedConfigType, Config $saved_config) {
    ...
    $config_template_file = DRUPAL_ROOT . '/' . drupal_get_path('profile', 'isa') . '/configbit/isa_config_templates/' . $supportedConfigType['config_template_file'];
  ...
  /**
   * Validate dependencies for isa config template bit data.
   *
   ...
   *   Config bit file source eg. isa.
   *
   * @return array|int
   *   array of config bit.
   *   FALSE if we do not have the config_bit in the config bit file.
   */
  public static function getConfigBit($config_bit_file_name, $type = 'profile', $project = 'isa') {
   ...
   */
  public static function doWeHaveThisConfigBit($config_bit_file_name, $type = 'profile', $project = 'isa') {
    // Generate full path to config file.
    $full_config_bit_file_name = drupal_get_path($type, $project) . '/' . $config_bit_file_name;
   ...
   */
  public static function getList($config_bit_file_name, $condition_name, $condition_value, $sublist = NULL, $type = 'profile', $project = 'isa') {
   ...
   *   The project.
   */
  public static function actionArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'isa') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   The project.
   */
  public static function actionUnArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'isa') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
    if (isset($config_bit_data['type'])
        && $config_bit_data['type'] = 'action'
        && isset($config_bit_data['action'])
   ...
   *   The project.
   */
  public static function actionAdd($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'isa') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   The project.
   */
  public static function actionRemove($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'isa') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
